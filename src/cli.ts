...
const SUBCOMMAND_USAGE: Record<string, string> = {
  ...
  "analyze-skill":
    "usage: analyze-skill <SKILL.md | skill-dir/ | glob>… [--strict] [--runtime] [--output-format text|json]   (ADVISORY token-free scan: flags a /sessions/... path handed to a file tool or dispatch/sub-agent output — denied on host-loop; AND interactive-artifact write-backs that are lost under Cowork)\n" +
    ...
    "       a DIRECTORY target scans the UNION of every contract-bearing markdown file present, not just SKILL.md — a plugin's dispatch/output contracts often live in agents/**/*.md or references/**/*.md (both walked RECURSIVELY; Claude Code discovers agents/sub/x.md too):\n" +
    ...
    "         · a skill dir INSIDE a plugin (SKILL.md present, walking up finds an enclosing plugin manifest) → + that plugin's root agents/**/*.md, references/**/*.md AND commands/**/*.md\n" +

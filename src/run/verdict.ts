...
/**
 * The persisted projection of {@link computeVerdict} for `RunResult.verdict` — call this at the
 * verdict-computation/persist point of every run/replay-lane producer (never for chat), right before
 * `result.json` is written, and store its return value. Never recomputes pass/fail independently:
 * `pass`/`exitCode` are the exact values `computeVerdict` returns.
 *
 * `signals[]` is collapsed into a flat `failures[]`:
 ...
 *  - an unanswered-gate salvage (`result.unansweredGate` set) is special-cased: `computeVerdict` only
 *    sees the generic `result:"error"` and emits a content-free `result_error`/`transport_error` signal
 *    for it, which would read as "run result was error" with no reason. Substitute the gate's own
 *    message (the decider's failure text, question embedded) so a salvaged run's persisted verdict
 *    actually names the gate reason instead of the generic placeholder.
 *
 * Empty `failures` on a pass. Plain JSON in shape (no functions, and `assertion: undefined` — set only
 * when no key was found — is dropped by `JSON.stringify` like every other optional field on this type).
 */
export function persistedVerdict(result: RunResult, lane: "live" | "replay"): NonNullable<RunResult["verdict"]> {
  const verdict = computeVerdict(result, lane);
  ...
  for (const s of verdict.signals) {
    ...
    // result_error/transport_error signal computeVerdict emits for the same result:"error" so a salvaged
    // run's failures[] doesn't carry two entries for one root cause.
  ...
  return { pass: verdict.pass, exitCode: verdict.exitCode, failures };
}

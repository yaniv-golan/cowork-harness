...
export function buildScaffold(eventsFile: string): string {
  ...
  // `artifacts: undefined` is this codebase's deliberate UNAVAILABLE sentinel (a replay result, or a run
  // whose workspace root was missing at collection — see inspect-view's
  // "artifacts: UNAVAILABLE ... NOT a run that produced nothing"), distinct from a verified-empty `[]`
  // (the run genuinely wrote no deliverables). Scaffolding zero file_exists asserts from `[]` is correct;
  // doing the same from `undefined` would silently encode "there were no deliverables to assert" into a
  // committed scenario, hiding outputs that were simply never observed. Refuse: skip the artifact asserts
  // and warn loudly instead, mirroring how the `partial` branch below degrades.
  const artifactsUnavailable = !partial && result.artifacts === undefined;
  ...
  const partialMarker = partial
    ...
    : "";
  const artifactsUnavailableMarker = artifactsUnavailable
    ? `# ARTIFACTS UNAVAILABLE — result.json has no artifacts manifest (a replay result, or a run whose\n` +
      `# workspace root was missing at collection), NOT a run that produced nothing. No file_exists assertions\n` +
      `# were scaffolded; this run may have written deliverables that were never observed. Resolve capture\n` +
      `# health (re-run, or inspect the run dir) and add file_exists asserts by hand before trusting this scenario.\n`
    : "";
  return (

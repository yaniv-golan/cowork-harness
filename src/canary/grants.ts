...
/**
 * Per-version sub-agent grant canary. A committed `{agentType → declaredTools}` map is the
...
export function loadGrantMap(path?: string): GrantMap {
  ...
  // A read/parse failure previously `catch { return {} }`, silently disabling drift
  // detection — corruption is exactly when the canary must fire, so THROW loud instead.
  let parsed: any;
  try {
  ...
  // `.grants ?? {}` previously coerced a missing/non-object key to an empty map (silent

...
```bash
npm install -g "cowork-harness@^3.9.0"    # puts the `cowork-harness` command on your PATH
```
...
> From a global install (`npm i -g "cowork-harness@^3.9.0"`), point at the package root instead:
...
> - **Invocation:** from a source checkout, `node dist/cli.js <cmd>` (or `npm link` to get the `cowork-harness` command); from a global install, `cowork-harness <cmd>`; the companion skill falls back to `npx "cowork-harness@^3.9.0"`.
...
The parts people ask about. **`package.json`'s `files[]` is the exhaustive, machine-readable source** —
...
| What ships | npm global (`npm install -g "cowork-harness@^3.9.0"`) | Source checkout (`git clone` + `npm ci`) |
|---|---|---|
...
and `run`ning the worked scenarios — pass them as `$(npm root -g)/cowork-harness/examples/scenarios/…`,
since a global install puts nothing in your working directory. The matrix, answer-policy and probe
...
arrives when the skill's first command self-bootstraps `npx "cowork-harness@^3.9.0"` — the last row stays
...
#    NOTE: examples/scenarios/ ships on npm too, but under $(npm root -g)/cowork-harness/ — so from a
#    global install prefix the path, or point this at your own scenarios/. See "What ships" above.
...
|---|---|---|
| `skill <folder> "<prompt>"` | Run a local skill/plugin folder once against the staged agent. An unanswered gate's `--on-unanswered` defaults adaptively (`prompt` only on a TTY outside CI; `fail` in CI **or** any non-TTY, including a piped run). Ablation, labelling, repeat and the three consent flags: see [Flags worth knowing](#flags-worth-knowing) | ad-hoc "is the skill alive / does it do X?" — the fast inner loop |
| `run <scenario.yaml \| dir/>` | Run authored scenarios with `assert:` + a CI-ready exit code; a decider can answer unscripted gates. Variance (`--repeat`), matrices and ablation: see [Flags worth knowing](#flags-worth-knowing) | you want a repeatable, **asserted regression test** — to **measure flakiness** instead of trusting one green — or to **test a compatibility matrix** (multiple baselines/models/skill variants) in one run |
...
| `lint <scenario.yaml \| dir/>…` | Check scenarios for silent false-greens — assertions on the wrong CI lane, mixed content/live keys, missing `controlOut`-required keys (bundled `scenario.py`; needs python3) — and that each file loads (the `run`/`record` loader's own checks, plus a named `baseline:`). `--json` / `--min-severity`: see [Flags worth knowing](#flags-worth-knowing) | before committing a new scenario or after changing assertions |
| `lint-skill <SKILL.md \| skill-dir/>…` | Lint a skill body (and any sibling `hooks.json`) for two Cowork host-loop footguns, statically resolve pinned `subagent_type` values against the enclosing plugin, and size the skill corpus against the `critique` evidence ceiling (bundled `scenario.py`; needs python3). Severities and `--strict`: see [Flags worth knowing](#flags-worth-knowing) | authoring or reviewing a skill before a paid Cowork host-loop run exposes the footgun, or before a pinned `subagent_type` typo breaks a `Task` dispatch |
...
| `analyze-skill <SKILL.md \| skill-dir/ \| glob>…` | Token-free ADVISORY scan for two defect classes: a `/sessions/...` path handed to a file tool or dispatch output (DENIED on host-loop), and an interactive artifact whose write-back is silently lost under Cowork. Ignore markers, glob/walk rules, `--strict` and `--runtime`: see [Flags worth knowing](#flags-worth-knowing) and [docs/subagents.md](./subagents.md#static-path-fidelity-check-analyze-skill) | catching the exact "skill hands `/sessions/...` to a file tool" defect — or an artifact whose Submit silently fails under Cowork — statically, before paying for a live run to discover it |
| `probe-dispatch <skill-dir> "<prompt>"` | Cheap single-dispatch mechanics probe — a thin wrapper over `skill` that scopes a prompt to one `Task` dispatch and prints just that dispatch's `{resolvedAgentType, pathDenials, delivered}`. Tier defaults and `--expect-write`: see [Flags worth knowing](#flags-worth-knowing) | "did THIS dispatch resolve to the type I expect, avoid a path denial, and actually deliver its write?" without hand-writing a scenario or reading `trace --view dispatches` |
...
| `status <run-id \| run-dir> [--follow]` \| `status --latest-for <scenario>` | Check whether a background run is alive (state/elapsed/tool counts) by reading `status.json` — no `ps aux` needed. Stream semantics, run-dir resolution and `--latest-for`: see [Flags worth knowing](#flags-worth-knowing) | a **driving agent** (or script) checking on a run it launched in the background, or locating the run it just kept |
| `stats [<scenario>]` | Queryable summary over every indexed `run`/`skill`/`record` invocation — run count, pass rate, cost/duration/token/turn p50/p95, `total=` group spend (the only figure that prices a `critique` whole), last-green timestamp. Grouping and filters: see [Flags worth knowing](#flags-worth-knowing) and [docs/stats.md](./stats.md) | "is this scenario flaky/expensive over time?" without hand-aggregating `result.json` files |
...
| `critique <skill-folder>` | **EXPERIMENTAL.** Run a skill, ask the agent what confused it, then grade that self-report against a frozen record of the run — blinded evaluator, mechanical citation checking. A discovery instrument, **never a gate** (findings always exit 0), and advisory: the skill under review can steer the grade. `--corpus-only` skips both turns entirely and prints just the evidence-corpus size, NO SPEND. Accepted flags, tiers and cost accounting: see [Flags worth knowing](#flags-worth-knowing) and [docs/critique.md](./critique.md) | "what confused the agent about my skill?" — including "does it even see the attached document?"; `--corpus-only` for "how close is this skill to the evidence ceiling?" before paying for either |
...
| `init-redact [--force]` | Copy the packaged reference `.cowork-redact.json` (local-path prefixes incl. macOS temp roots and slugged home segments + a generic email regex) into the cwd; refuses to overwrite without `--force`. Review + tailor before recording; re-run with `--force` to pick up rules added in a later release | `record` warned that a `hostloop`/`protocol` recording has no redaction policy |
...
The full list is always `<command> --help`. This section carries the detail the table above deliberately omits.
- `skill` / `run` / `probe-dispatch`: `--ablate-skill` runs **this one invocation** with the skill removed — the control arm of a with/without comparison, **not both arms** (run it a second time without the flag for the treatment arm; composed with `--repeat N` it produces N *ablated* runs, and the rollup's verdict line names the arm — `PASS [ABLATED — control arm] — 5/5 passed` — so a one-armed batch cannot be misread as a finished A/B); `--repeat N` (2-100) runs the same skill+prompt N times and aggregates a variance rollup ("did this finding reproduce, or did it pass once?"); `--compact`/`--demo` trim output for shareable screenshots/GIFs; `--allow-missing-capability` stops a capability false-negative on the lean `core` image from failing the verdict (the open-ended equivalent of asserting `allow_missing_capability: true`); `--allow-host-writes` consents to a writable `hostloop`-fidelity connected folder (the same consent as clicking "connect folder" in Desktop — no effect off `hostloop` or without a writable `--folder`); `--allow-host-hooks` consents to running a staged plugin's hooks as native host processes at `protocol` (L0 passes `--plugin-dir`, so the CLI executes them on your machine with no sandbox — refused loud otherwise; plugins without hooks need no opt-in). The scenario-file equivalent of the last one is the top-level `allow_host_hooks: true`, which needs **cowork-harness ≥ 3.0.0** — an older CLI hard-errors `Unrecognized key` (exit 2) rather than defaulting it.
...
- `chat`: `--upload <file>` / `--folder <dir>` (repeatable) attach files/project folders; `--verbose` shows thinking blocks + tool inputs; `--fidelity protocol|container|hostloop` only (no `microvm`/`cowork` in the REPL); `--allow-host-writes` and `--allow-host-hooks` carry the same consent meaning as on `skill`; `--raw` skips the control protocol for a native `docker run -it` (and therefore rejects `--upload`/`--folder`/`--plugin`/`--fidelity`).
...
- `trace`: the full view enum is `--view tools|questions|dispatches|tool-durations|tool-errors|files|usage|subagent-research` (pinned to `src/cli.ts`'s `VIEWS` by `test/trace-view-doc-sync.test.ts`). `--translate-paths` rewrites VM paths to host paths in the text `tools`/default views; `--full-results` captures the full input + result of **every** tool call — successful ones too, not just errors — so an external grader can ground a self-critique finding against the call it cites. `--view tool-errors` gives the full stderr of every **failed** call, and `--view subagent-research` includes a search made by a *nested* dispatch, attributed to its nearest ancestor and marked `[via nested agent …]` (live/record lane).
- `lint`: takes files or a directory of `*.yaml`/`*.yml`. Besides the bundled linter's own rules it runs the scenario loader `run`/`record` use, so every file the loader would reject is an ERROR `scenario-invalid`, and a `baseline:` naming no baseline this installed CLI ships is an ERROR `baseline-unknown` — machine-dependent inputs (the session file, an absolute baseline path, environment variables) are not checked. `--json` emits findings as machine-readable JSON instead of the text report; `--min-severity ERROR|WARN|INFO` drops findings below a floor **before** both the report and the exit computation (so `--strict --min-severity ERROR` behaves as a plain lint), which is the way to mute the unconditional INFO advisories in CI.
- `lint-skill`: the two host-loop footguns (a `${CLAUDE_PLUGIN_ROOT}` path used in an in-VM bash context; a hook command that exports an env var or writes into `/tmp` for the in-VM agent) are WARN-only. Of the three `subagent_type` outcomes, an in-plugin-prefixed agent missing from the enclosing plugin's fully-enumerated `agents/*.md` (`subagent-type-not-found-in-plugin`) is a **provable typo and is WARN too**, while a cross-plugin (`subagent-type-unresolvable`) or unknown-bare (`subagent-type-unknown`) value stays INFO — there is no built-in agent-type registry to disprove it against. Corpus sizing covers `SKILL.md` + every file under `references/` (any extension) + every `agents/**.md` a plugin skill can dispatch, against the 512 KiB `critique` evidence ceiling: `skill-corpus-near-evidence-ceiling` at ≥ 80% is INFO, `skill-corpus-over-evidence-ceiling` past it is **WARN**, so a corpus a critique would cut fails `--strict`. Pass `--strict` (the CI-recommended invocation; plain `lint-skill` is advisory-only) to fail on any WARN, or `--json` for machine-readable findings. **It counts the same four corpus classes a live `critique` packages** (see [docs/critique.md](./critique.md#multi-skill-plugins---skill)), including every plugin-root `references/` file the skill links. The one clause it cannot mirror is the run-dependent one — a root reference included only because the graded agent READ it — since a static lint has no run to read, so its number can under-report there and a real critique report's `corpusCuts`/`corpusOmitted` stay the authority. It also diverges from a real critique on four measured axes: it counts an untracked file staging would drop and a symlink pointing outside the plugin that the packager's containment rule refuses (both over-counts), and it cannot see a plugin-root reference read at run time and sums `st_size` where the packager measures decoded UTF-8 length (both under-counts; the byte axis moves only on a file that is not valid UTF-8 — clean multibyte text round-trips exactly). `critique --corpus-only` closes three of those four by being the packager itself — same call, same filter, same ceiling — over an empty run, at no spend; see [Knowing before you pay](./critique.md#knowing-before-you-pay).
- `analyze-skill`: the `--output-format json` shape, the ignore-marker syntax and the directory-walk rules are specified in [docs/subagents.md](./subagents.md#static-path-fidelity-check-analyze-skill). It accepts multiple positionals (files, dirs, or a simple `*`/`**` glob), walked recursively across a plugin's `SKILL.md`/`agents/`/`references/`/`commands/`. Findings print but exit 0 by default; `--strict` (the CI-recommended invocation) fails on any unsuppressed **`error`-severity** finding — advisory findings never gate, and `ok` mirrors the exit code rather than the finding count. Three per-file ignore markers (`ignore-next-line`, `ignore-start`/`ignore-end`, file-wide `ignore`) and an `--output-format json` payload are supported. The artifact detector statically analyzes `.html`/`.js`/`.py` sources under the target — `.ts`/`.tsx`/`.jsx` are **out of scope** (the in-process parser cannot read them, so a directory walk over a TypeScript generator reports nothing; naming the file as a positional is what surfaces the skip under `unscannedArtifactSources`) — for a relative `fetch`/XHR/`sendBeacon`/`<form method=post>` write-back that silently fails when the artifact is served from Cowork's own origin. `artifact-write-back-lost` gates under `--strict`; `artifact-write-back-suspect` is advisory; a candidate that can't be parsed is a could-not-verify exit `3`. `--runtime` adds an optional headless-DOM confirmation (needs `jsdom`) that *observes* the lost write-back.
- `probe-dispatch`: a THIN wrapper over `skill` (fidelity `container`/`microvm`/`hostloop`, default `hostloop`) — no new data model, a pure projection of the same `RunResult` `skill` already produces. `--expect-write <suffix>` narrows `delivered`; "one dispatch" is prompt-scoped, **not enforced**; `--output-format json` for machine consumption. It also inherits the common decider/answer flags — `--decider-cmd`, `--decider-dir`, `--on-unanswered`, `--ablate-skill`.

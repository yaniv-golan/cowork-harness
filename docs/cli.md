...
| `lint <scenario.yaml \| dir/>…` | Check scenarios for silent false-greens — assertions on the wrong CI lane, mixed content/live keys, missing `controlOut`-required keys (bundled `scenario.py`; needs python3). `--json` / `--min-severity`: see [Flags worth knowing](#flags-worth-knowing) | before committing a new scenario or after changing assertions |

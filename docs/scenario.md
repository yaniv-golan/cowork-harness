...
| `semantic_matches: {rubric, min_pass?}` | a pinned LLM judge grades each fixed `rubric` claim against the run's answer; passes iff ≥ `min_pass` claims pass (default: all) — **live lane only** (an LLM judge call); skipped-loud on replay |

...
| `no_delete_in_outputs: true` | no delete op (`rm`/`mv`/…) touched `mnt/outputs` (Cowork's outputs mount fails `unlink`/`rmdir` with `EPERM`) — **only `true` is valid**; writing `false` is rejected by the schema. **Omitting the key does NOT allow deletes**: a detected delete still fails the run via the `outputs_delete` verdict signal, which fires precisely *because* the key was not authored — authoring it turns that signal into an explicit assertion. To accept an intended delete, use `allow_outputs_delete: true`. Detects operations that UNLINK a name — a post-run bash-command scan plus a filesystem diff of `outputs/` per turn, not mount-level enforcement, so a green means none was *detected*, not that the mount enforced anything. A detected delete **fails** when the filesystem diff of `outputs/` proves it (a path present at turn start is gone), when a delete in command or call position has an `outputs/` path as its own operand (`rm`/`rmdir`/`unlink`/`shred -u`/`find … -delete`, `os.remove(…)`/`shutil.rmtree(…)`/`Path(…).unlink()`, or a move out of outputs), or when the diff could not verify the turn. A hit that rests only on the detector's inference — an unprovable target, a relative `cd` into outputs, or an outputs path that merely shares a statement with a word like `rm` (a Python variable `rm = json.load(open(".../outputs/r.json"))`, quoted prose, a `sed`/`grep` pattern, a trailing comment) — with a clean diff is the `outputs_delete_unconfirmed` **warn** instead (an authored key passes on it; the `outputs_delete_unconfirmed` warn is still raised in the run output, and the hit is also kept as the assertion's evidence in the JSON envelope). A *statement* is one fragment of the command split on newline, `;`, `&&` and `||` (quote-blind, after comments are stripped and same-command `VAR=value` assignments expanded one level), so real deletes in a loop body, after a `cd`, through a chained or computed variable, or as `p = …` then `os.remove(p)` land in the warn tier; and quoted text that the quote-blind split turns into a delete command with an outputs operand (`echo 'note; rm mnt/outputs/x'`) still fails — waive that with `allow_outputs_delete`. Emptying a file in place (`truncate`, `>`, `shred` without `-u`) is not a delete and is permitted by Cowork, so it is not flagged |

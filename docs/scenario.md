...
| `no_delete_in_outputs: true` | no delete op (`rm`/`mv`/…) touched `mnt/outputs` (Cowork's outputs mount fails `unlink`/`rmdir` with `EPERM`) — **only `true` is valid**; writing `false` is rejected by the schema. **Omitting the key does NOT allow deletes**: a detected delete still fails the run via the `outputs_delete` verdict signal, which fires precisely *because* the key was not authored — authoring it turns that signal into an explicit assertion. To accept an intended delete, use `allow_outputs_delete: true`. Detects operations that UNLINK a name — a post-run bash-command scan plus a filesystem diff of `outputs/` per turn, not mount-level enforcement, so a green means none was *detected*, not that the mount enforced anything. A detected delete **fails** when the filesystem diff of `outputs/` proves it (a path present at turn start is gone), when the flagged delete statement itself names an `outputs/` path (or moves something out of outputs), or when the diff could not verify the turn. A hit that rests only on the detector's inference — an unprovable target such as a Python variable named `rm` next to an outputs path, or a relative `cd` into outputs — with a clean diff is the `outputs_delete_unconfirmed` **warn** instead (an authored key passes on it; the `outputs_delete_unconfirmed` warn is still raised in the run output, and the hit is also kept as the assertion's evidence in the JSON envelope). A *statement* is one fragment of the command split on newline, `;`, `&&` and `||` (quote-blind, after comments are stripped and same-command `VAR=value` assignments expanded one level), so real deletes in a loop body, after a `cd`, through a chained or computed variable, or in a multi-line `python3 -c` body land in the warn tier; and `rm = open(".../outputs/r.md")` quoted prose like `echo 'rm outputs/x'`, a trailing comment on a real command (`rm -rf build # … mnt/outputs`, since only whole-line comments are stripped), or a `sed`/`grep` pattern containing a delete word next to an outputs path (`sed -i '/rm/d' mnt/outputs/x.md`) still fails — waive those with `allow_outputs_delete`. Emptying a file in place (`truncate`, `>`, `shred` without `-u`) is not a delete and is permitted by Cowork, so it is not flagged |
...
- `outputs_delete_unconfirmed` (**warn**, live lane) — a delete-shaped command near `mnt/outputs` that
  nothing confirms: no output present at turn start was deleted, and no flagged delete statement names an
  outputs path — the flag rests on the detector's inference (an unprovable target such as a Python
  variable named `rm`, or a relative `cd` into outputs). A file created *and* deleted within the turn is
  invisible to the filesystem diff, so this can still be a real delete (in a loop, after a `cd`, through a
  chained or computed variable): read the command. `allow_outputs_delete` waives it.
- `outputs_diff_unavailable` (**warn**, live lane) — the per-turn filesystem diff of `outputs/` could not
  ...
  undetected. A text hit on such a turn fails as `outputs_delete` instead.
...
- `scan_unavailable` (**warn**) — post-run scan evidence unavailable (`RunResult.scan` undefined); the
  ...
  filesystem diff still ran, and a delete it proves still fails as `outputs_delete` (assert

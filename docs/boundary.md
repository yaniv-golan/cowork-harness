...
- **`hostloop`**: against Desktop **2.7032.0+** the agent runs at `/var/empty`, so a relative path is
  refused — write the absolute host outputs path the prompt names (`<outputs>/artifacts/<...>`). Before
  2.7032.0 the file-tool cwd was the host outputs dir and cwd-relative `artifacts/<...>` worked. `/sessions/...` is **denied unconditionally** by the path gate above — it
...
- **Read-only mounts ARE enforced; outputs delete-deny is still post-hoc.**
  ...
  - Unresolved/command-substituted delete targets (other than the recognized `mktemp` idiom) still always flag — but a flag is only *fail*-authority when something confirms it: a per-turn filesystem diff of `outputs/` proving a path present at turn start is gone, a delete in command/call position whose own operand is an `outputs/` path, or a turn whose diff could not verify. A flag resting only on inference (an unprovable target, a relative `cd`) with a clean diff is the `outputs_delete_unconfirmed` warn. The diff cannot see a file created and deleted within one turn, so real deletes can land in that warn: a loop body whose operand is the loop variable (`for f in …; do rm "$f"; done`), a `cd` then a relative path, chained variables (`A=…; B=$A/x; rm "$B"`), a Python path held in a variable set on another line (`p = …` then `os.remove(p)`, or `for p in …:` then `p.unlink()`), wrappers with flag combinations the classifier does not model (`sudo -Hu user rm`, `git -C dir rm`), and calls outside the modelled set such as Node's `fs.promises.rm(…)`.

...
The skill **self-bootstraps the CLI**: if `cowork-harness` isn't on your PATH it falls back to `npx "cowork-harness@^3.3.0"` (a version floor that fails loud rather than silently fetching a too-old CLI; Node ≥ 22). Tiers above `protocol` still need Docker/Lima and a Claude Desktop agent binary — see the prerequisites below.
...
| What ships | npm global (`npm install -g "cowork-harness@^3.3.0"`) | Source checkout (`git clone` + `npm ci`) |
|---|---|---|
...
above becomes available once the skill's first command self-bootstraps `npx "cowork-harness@^3.3.0"` — see

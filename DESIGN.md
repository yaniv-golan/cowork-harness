...
### Control protocol — VERIFIED end-to-end against the live host CLI (macOS build 2.1.177+; the staged in-VM agent that L1/L2 run is **2.1.202**, baseline **`desktop-1.19367.0`** — a fresh live end-to-end pass across the `protocol`, `container`, and `hostloop` tiers was run against this baseline on 2026-07-10, superseding the prior `1.17377.1` pin; the intervening `1.17377.1`→`1.19367.0` bumps were byte/behaviourally-identical asar analysis, now confirmed live)
...
### Spawn contract + host-loop vs VM-loop (binary-verified through asar 1.17377.1; asar analysis since carried through 1.19367.0 — the full spawn contract is byte-identical across the 1.18286.0→1.18286.2 bump, and behaviorally identical across the 1.18286.2→1.19367.0 Vite code-split/re-minify: the value-resolved contract is unchanged and only minified symbol names + bundle layout moved, re-verified via the structural anchors and the live-asar spawn-contract tests — the live end-to-end pass now covers `1.19367.0` too (see the "Control protocol" note above), superseding the prior `1.17377.1` pin)

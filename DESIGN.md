...
This document is the reference for *how faithful* each tier is, *what we deliberately don't reproduce*, and *why the chosen seams keep parity cheap to maintain*. Everything here is grounded in analysis of the live Claude Desktop `app.asar` (build 1.12603.1; the spawn contract and gates were re-verified at this build — some earlier prose was first written against 1.11847.5) and the on-disk runtime state on macOS.
...
- In-VM agent: `~/Library/Application Support/Claude/claude-code-vm/<ver>/claude` (currently **2.1.170**, an **ELF aarch64** binary), spawned by the host **in cowork mode via the `CLAUDE_CODE_IS_COWORK=1` env var** — *not* a `--cowork` flag (that flag is plugin-scope and the staged agent rejects it; see the control-protocol note below).
...
VOLATILE (changes per release; lives in baselines/*.json, regenerated by cowork-sync)
  - agentVersion
  - coworkFlag + BG env-strip list
  - mountLayout
  - egress.allowDomains + networkMode + requireFullVmSandbox
...
*.claude.ai (assets / downloads / pivot / preview)  sentry.io  statsig*
```
...
### Control protocol — VERIFIED end-to-end against the live host CLI (macOS build 2.1.174; the staged in-VM agent that L1/L2 run is 2.1.170)

...
### Control protocol — VERIFIED end-to-end against the live host CLI (macOS build 2.1.177+; the staged in-VM agent that L1/L2 run is 2.1.197, baseline `desktop-1.17377.1` — the live end-to-end pass remains pinned here; asar/baseline analysis has since been carried through `desktop-1.19367.0` with no protocol-affecting deltas, pending a fresh live re-run against it)

...
metadata:
  ...
  version: 1.0.5
  tracks-harness: cowork-harness 1.0.5 (baseline desktop-1.20186.9)
---
...
> **Version note:** the facts and `file:line` pointers here track `cowork-harness 1.0.5` (baseline
...
- **CLI on PATH, recent enough?** Run `cowork-harness --version` — this skill needs **≥ 1.0.5**. If it's missing or older, prefix every command with the version floor `npx "cowork-harness@>=1.0.5" <cmd>` (Node ≥ 20), or install once with `npm i -g "cowork-harness@>=1.0.5"`. **Pin `@>=1.0.5`, never `@latest`** — `@latest` can silently fetch an older CLI and the new commands fail as "unknown command", whereas the floor **fails loud** if no compatible version is published.

  What the ≥ 1.0.5 floor gates, by release:

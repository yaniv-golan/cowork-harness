...
metadata:
  ...
  version: 0.10.0
  tracks-harness: cowork-harness 0.10.0 (baseline desktop-1.14271.0)
---
...
> **Version note:** the facts and `file:line` pointers here track `cowork-harness 0.10.0` (baseline
...
- **CLI on PATH, recent enough?** Run `cowork-harness --version` — this skill needs **≥ 0.10.0** (the commands/assertions it teaches: `assertions --list`, `scaffold <run-id>`, `trace --view dispatches`, `artifact_json` incl. the `in:` operator, `verify-cassettes`, batch `record <dir>`/`--rerecord-stale`, record-time redaction, multiSelect/`answer:`, `verify-run` (incl. answer-coverage when a scenario declares `answers:`), `record --max-artifact-bytes`, answering gates live during a recording (`record --decider-dir`/`--decider-llm`/`--on-unanswered first`), `verify-cassettes --allow-domain`/`--allow-email`/`--allow-file`, scenario `skills:` staleness scoping, `chat --plugin`, and `/help` in the chat REPL). If it's missing *or older than 0.10.0*, prefix every command with `npx` using a version floor: `npx cowork-harness@>=0.10.0 <cmd>` (Node ≥ 20). The floor matters — plain `@latest` would silently fetch an older CLI and the new commands would fail as "unknown command"; `@>=0.10.0` instead **fails loud** if no compatible version is published. To install once instead: `npm i -g cowork-harness@latest`.

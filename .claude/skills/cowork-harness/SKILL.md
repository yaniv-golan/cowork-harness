...
metadata:
  ...
  version: 0.3.0
  tracks-harness: cowork-harness 0.3.0 (baseline desktop-1.12603.1)
---
...
> **Version note:** the facts and `file:line` pointers here track `cowork-harness 0.3.0` (baseline
...
- **CLI on PATH, recent enough?** Run `cowork-harness --version` — this skill needs **≥ 0.3.0** (the commands/assertions it teaches: `assert --list`, `scaffold`, `trace --dispatches`, `artifact_json` incl. the `in:` operator, `verify-cassettes`, batch `record <dir>`/`--rerecord-stale`, record-time redaction, multiSelect/`answer:`). If it's missing *or older than 0.3.0*, prefix every command with `npx` using a version floor: `npx cowork-harness@>=0.3.0 <cmd>` (Node ≥ 20). The floor matters — plain `@latest` would silently fetch an older CLI and the new commands would fail as "unknown command"; `@>=0.3.0` instead **fails loud** if no compatible version is published. To install once instead: `npm i -g cowork-harness@latest`.

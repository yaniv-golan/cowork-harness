...
| `no_delete_in_outputs: true` | no delete op touched `mnt/outputs` — **only `true` is valid**; `false` is rejected. **Omitting it does NOT allow deletes** — a detected delete still fails via the `outputs_delete` verdict signal, which fires *because* the key was not authored; use `allow_outputs_delete: true` to accept an intended one. Detection is a post-run bash-command scan plus a per-turn filesystem diff of `outputs/`, not mount enforcement, so a green means none was *detected*. Fails when the diff proves the delete, when the flagged delete statement itself names an `outputs/` path, or when the diff could not verify; a hit resting only on the detector's inference (e.g. a Python variable named `rm`) with a clean diff passes, and the `outputs_delete_unconfirmed` warn is still raised in the run output — see that code for the classes of real delete that land there |
...
| `outputs_delete` | fail | An unauthorized delete touched `mnt/outputs`, confirmed: the per-turn filesystem diff proves it, the flagged delete statement itself names an `outputs/` path, or the diff could not verify the turn. Authoring `no_delete_in_outputs` moves it into that assertion; `allow_outputs_delete` waives it |
| `outputs_delete_unconfirmed` | warn | A delete-shaped command near `mnt/outputs` nothing confirms: no output present at turn start was deleted and no flagged statement names an outputs path (e.g. a Python variable named `rm`). A *statement* is a fragment split on newline/`;`/`&&`/`\|\|`, quote-blind, so real deletes in a loop, after a `cd`, through chained or computed variables, or in a multi-line `python3 -c` body also land here — read the command. Raised even when `no_delete_in_outputs` is authored (that assertion passes on it). Kept false positives that still fail `outputs_delete` because the statement names outputs: `rm = open(".../outputs/r.md")`, quoted prose, a trailing `#` comment, a `sed`/`grep` pattern with a delete word. Waive: `allow_outputs_delete` |

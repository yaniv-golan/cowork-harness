...
> From a global install (`npm i -g "cowork-harness@>=0.29.0"`), point at the package root instead:
...
> - **Invocation:** from a source checkout, `node dist/cli.js <cmd>` (or `npm link` to get the `cowork-harness` command); from a global install, `cowork-harness <cmd>`; the companion skill falls back to `npx "cowork-harness@>=0.29.0"`.
...
The skill **self-bootstraps the CLI**: if `cowork-harness` isn't on your PATH it falls back to `npx "cowork-harness@>=0.29.0"` (a version floor that fails loud rather than silently fetching a too-old CLI; Node ≥ 20). Tiers above `protocol` still need Docker/Lima and a Claude Desktop agent binary — see the prerequisites below.
...
above becomes available once the skill's first command self-bootstraps `npx "cowork-harness@>=0.29.0"` — see
...
| `--output-format json` | Stable machine envelope on stdout: `{tool, version, command, ok, results[], error}`; each result carries `verdict: {pass, signals[], guards[]}` so you can read per-result pass/fail **and why** without recomputing; errors are `{ok:false, error:{category,message,hint}}` — see [SPEC §11](./SPEC.md) |
...
- `record`/`replay`: `--decider-llm`/`--decider-dir` answer gates live during recording; `--rerecord-stale`/`--concurrency <N>` batch re-record; `--assert-from <scenario.yaml>`/`--reassert` re-check the on-disk `assert:` instead of the frozen one; `--strict`/`--fail-on-skill-drift` control staleness handling on replay; `--no-redact` skips record-time redaction; `--allow-failing` relaxes the post-run verdict gate; `--dry-run` resolves without recording; `replay --best-effort-future-cassette` lets a cassette from a newer format version replay anyway (warn instead of the default hard error).
...
- **Sub-agents & skills:** `subagents[]` now also carries `prompt`, `model`, `output`, and `attributedSkillId`; `skillActivity[]` attributes tool calls to whichever skill was active when they ran; `referencesRead[]` lists the skill's `references/*`/`scripts/*` files the agent's **main agent** actually **Read** (a progressive-disclosure signal — `SKILL.md` is delivered whole so it never appears, `assets/` and sub-agent reads aren't tracked; present on live and replay).
  ...
  live:
    ...
    steps:
      ...
      - name: Stage the agent binary (official channel, sha256-verified — see docs/maintenance.md)
        run: |
          V=2.1.202   # match your scenario's pinned baseline's agentVersion
...
**Verified end-to-end against the live staged agent (VM ELF 2.1.202 / native host app 2.1.205 / asar 1.20186.0).** Host-side parity (egress/gates/mount/web_fetch facts) re-derived via `sync` — no unknown deltas; `asarFingerprint` moved (a 1.19367.0→1.20186.0 minifier re-anchor: hoisted helpers became namespace-method calls, spreads became export aliases — value-resolved contract unchanged, extractor re-anchored in `src/sync/cowork-sync.ts`), and `nativeStagedPath` moved to the natively-staged `2.1.205` (the native macOS app and the VM ELF version on independent cadences). The reconstructed prompt appends were re-checked against the new asar: the system-prompt source added a trailing `{{modelIdentity}}` placeholder, but it is deployment-gated and stripped on the first-party Cowork emulated here, so the rendered prompt is byte-identical — the load-bearing "is NOT Claude Code" correction, the host-loop `## Shell access` generator, and the web_fetch/mount facts all still hold. The live scenario suite (`protocol` + `container` + `hostloop` tiers) passes against this baseline (fresh live re-run 2026-07-11; 5/6 scenarios, `example-pdf-skill` needs the full-parity image). The latest shipped baseline — what `baseline: latest` resolves to (`cowork-harness list`) — is **`desktop-1.20186.1`** (the committed baseline file and version-lockstep checks track it; it additionally carries the synced per-model effort config, and there are no known spawn-contract deltas — the `1.20186.0` bundle's anchor drift was benign and is resolved in `src/sync/cowork-sync.ts`).

- ✅ **Three isolation tiers (L0/L1/L2) + two loop-mode overlays** — see [Fidelity tiers](#fidelity-tiers-pick-per-scenario--per-ci-job) for the full decision guide; in short:
  - `protocol` (L0, control loop only), `container` (L1, sandboxed arm64 + per-run default-deny egress sidecar), `microvm` (L2, real Apple-VZ Linux microVM + guest firewall).
  - `hostloop` (production split-execution: native host agent process, file tools routed host-side via the workspace SDK-MCP server into a Docker VM sidecar) and `cowork` (auto-picks `hostloop` vs `container` the way Cowork does, via gate `1143815894`).
  - Egress enforced at `container`/`microvm`/`hostloop`; `boundary-check` reports **ALL CONSTRAINTS ENFORCED**.
...
- ✅ **Answering live questions, every way you'd need** — four independent channels, all incapable of silently picking option 1 (an unhandled question always fails loud):
  - `--decider-llm --intent "<one line>"` — a model picks per question, steered by your test intent (the ergonomic default); defaults to **Sonnet** (a weaker model prose-declines ambiguous judgment gates → fail-loud); override with `--decider-model <id>` to pin a cheaper or stronger model. It replies with the option **number** (mapped to the exact label, so it can't whiff by parroting label text); an out-of-set reply fails loud, and non-determinism is flagged so a green isn't mistaken for a scripted pass. ⚠️ A *decided* gate whose answer feeds a **semantic** assertion can still false-green — script those instead.
  - `--answer-policy <yaml>` / `--answer "rx=c"` — declarative regex→label, deterministic for CI (accepts a **stable partial anchor** for skills whose labels drift run-to-run).
  - `--decider-cmd '<helper>'` — custom logic via the Python `serve_decider(fn)` adapter, which pre-builds the wire loop so the helper writes only the decision.
  - `--decider-dir <dir>` — the *driving agent* answers each gate **in-band** with full context: it arms a Monitor on `cowork-harness gates <dir> --follow` and replies with `cowork-harness answer <dir> --gate <N> --choose <label>`; the session-under-test stays live, no resume/re-worded question (binary-verified).
  - **multiSelect gates** are answerable on every channel — repeat `--choose` (or, for `--decider-llm`, a comma-list of option numbers; or a JSON-array reply from a `--decider-cmd` helper / hand-written `resp-N.json`); all deliver the binary-verified `", "`-joined wire shape. Every channel keeps stdout free, so all compose with `--output-format json`. Validate any decider in ~2s with `cowork-harness decide`.

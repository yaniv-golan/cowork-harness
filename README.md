...
| `analyze-skill <SKILL.md \| skill-dir/>` | Token-free ADVISORY scan: flags a `/sessions/...` path handed to a file tool or used as a dispatch/sub-agent output path in a SKILL.md — that path class is DENIED on host-loop (reuses the harness's own ported `/sessions` path-gate predicate as the deny decision; only the SKILL.md text extraction is heuristic, and it's conservative — bash fences, negation idioms ("never write to..."), and plain prose are exempt). Findings print but exit 0 by default; `--strict` fails (exit 1) on any finding instead (mirrors `lint-skill --strict`). Three ignore markers, three scopes: `analyze-skill: ignore-next-line` silences just the following line, `analyze-skill: ignore-start`/`ignore-end` silences a fenced block (an unclosed `ignore-start` is itself a gating `unclosed-ignore-fence` finding — never a silent notice), and the file-wide `analyze-skill: ignore` silences every finding for the file, even under `--strict` (all three: bare, or in an HTML comment) — see [docs/subagents.md](./docs/subagents.md#static-path-fidelity-check-analyze-skill) | catching the exact "skill hands `/sessions/...` to a file tool" defect statically, before paying for a live host-loop run to discover it |

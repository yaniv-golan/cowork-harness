...
| `analyze-skill <SKILL.md \| skill-dir/>` | Token-free ADVISORY scan: flags a `/sessions/...` path handed to a file tool or used as a dispatch/sub-agent output path — that path class is DENIED on host-loop (reuses the harness's own ported `/sessions` path-gate predicate as the deny decision; only the text extraction is heuristic, and it's conservative — bash fences, negation idioms ("never write to..."), and plain prose are exempt). A DIRECTORY target scans the UNION of every contract-bearing markdown file present (top-level `SKILL.md` + `references/**`; a plugin root's `agents/*.md` + `references/**`/`commands/*.md`/`skills/*/SKILL.md`(+`references/**`); a skill-dir-inside-a-plugin also pulls the enclosing plugin's `agents/*.md`) — deduped by resolved path; zero scannable files is a usage error (exit 2), never a silent clean pass. Findings print but exit 0 by default; `--strict` fails (exit 1) if ANY scanned file has an unsuppressed finding (mirrors `lint-skill --strict`). Three ignore markers, three scopes, applied PER FILE: `analyze-skill: ignore-next-line` silences just the following line, `analyze-skill: ignore-start`/`ignore-end` silences a fenced block (an unclosed `ignore-start` is itself a gating `unclosed-ignore-fence` finding — never a silent notice), and the file-wide `analyze-skill: ignore` silences every finding for that one file, even under `--strict` (all three: bare, or in an HTML comment). `--output-format json` nests results under `files: [{file, findings, suppressed}]` (plus `scanned`/`unscanned`) — see [docs/subagents.md](./docs/subagents.md#static-path-fidelity-check-analyze-skill) | catching the exact "skill hands `/sessions/...` to a file tool" defect statically, across a whole plugin's contract surface, before paying for a live host-loop run to discover it |

...
| `analyze-skill <SKILL.md \| skill-dir/>` | Token-free ADVISORY scan: flags a `/sessions/...` path handed to a file tool or used as a dispatch/sub-agent output path in a SKILL.md — that path class is DENIED on host-loop (reuses the harness's own ported `/sessions` path-gate predicate as the deny decision; only the SKILL.md text extraction is heuristic, and it's conservative — bash fences, negation idioms ("never write to..."), and plain prose are exempt). Findings print but exit 0 by default; `--strict` fails (exit 1) on any finding instead (mirrors `lint-skill --strict`); a line containing `analyze-skill: ignore` (bare or in an HTML comment) in a SKILL.md silences every finding for that file, even under `--strict` | catching the exact "skill hands `/sessions/...` to a file tool" defect statically, before paying for a live host-loop run to discover it |
